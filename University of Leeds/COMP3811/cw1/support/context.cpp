...
{
	// Note to Apple users:
	// The shaders use layout(binding=X), which was added in OpenGL 4.2 / GLSL
	// version 420. You may need to work around that on Apple systems.
...
void Context::draw( Surface const& aSurface )
{
	...
	// Should we use an intermediate PBO here? Unsure ... we have to wait for
	// the copy to finish either way. (glTexSubImage2D starts taking quite a
	// bit of time for higher resolution images...)
...
#	if defined(__APPLE__)
	std::printf( "WARNING: __APPLE__ defined. Things may not work, and things that appear to work are still untested.\n" );
#	endif // ~ __APPLE__
...
GLuint Context::create_tex_image_( std::size_t aWidth, std::size_t aHeight )
{
	...
	// Use immuatable texture storage, if possible. Immutable texture storage
	// was added in OpenGL 4.2, so it isn't available on Apple-y platforms.
	// Fallback to standard texture storage there.
#	if !defined(__APPLE__)
...
#	endif // ~ __APPLE__
	...
	// Make sure we use nearest filtering and clamp to edge
